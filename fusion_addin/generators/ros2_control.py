...
# joint_trajectory_controller's own `state_interfaces` parameter (distinct
# from an Actuator's command interface, or the <ros2_control> XML's per-joint
# <state_interface> list, which both accept "effort") only accepts
# {position, velocity, acceleration} -- confirmed for real against a live
# controller_manager: "effort" here raises "Entry 'effort' in parameter
# 'state_interfaces' is not in the set '{position, velocity, acceleration}'".
# Our hardware exposes position/velocity/effort but never acceleration, so
# the correct, always-valid subset for JTC's own config is just these two.
_JTC_STATE_INTERFACES = ("position", "velocity")
...
def generate_controllers_yaml(robot: Robot) -> str:
    ...
    if drivetrain is not None:
        ...
        for interface in _JTC_STATE_INTERFACES:
            lines.append(f"      - {interface}")
...
def generate_control_launch(robot: Robot) -> str:
    ...
    `robot_state_publisher`, `controller_manager`'s `ros2_control_node`
    (parameterized by `robot_description` plus this module's generated
    `controllers.yaml`), and spawns `joint_state_broadcaster` plus the
    arm/diff-drive controller via `ros2 run controller_manager spawner
    <name>` (expressed as launch `Node` actions, matching
    `ros2_control_demos`' launch file pattern).

    `robot_state_publisher` is included here, not left to a separately-run
    `display.launch.py` -- confirmed for real against a live
    `controller_manager` that this installed ros2_control version's
    `ros2_control_node` subscribes to the `/robot_description` *topic* for
    the URDF (logs "Subscribing to '/robot_description' topic") rather than
    relying solely on the `robot_description` parameter passed to it
    directly, and hangs indefinitely ("Waiting for data on 'robot_description'
    topic to finish initialization") with nothing to publish that topic if
    `robot_state_publisher` isn't running. Matches `ros2_control_demos`' own
    bringup launch files, which include `robot_state_publisher` for the same
    reason -- this wasn't a documented convention I'd originally cross-checked
    against, it's a real behavior confirmed by actually running it.
...
controllers.yaml), then spawns joint_state_broadcaster plus {second_controller}
